...
def get_args():
    ...
    parser.add_argument("--save_folder", type=str, default='./generated_images/icl_indomain')
...
if __name__ == "__main__":
    ...
    for index, subjectname in enumerate(tqdm(subjectnames)):
        ...
        image = [Image.open(image_path).convert("RGB") for image_path in image_paths][:num_images_used]

        ict_example = f"This is a photo of Max: <image>. This is another photo of Max: <image>\n"
        # duplicate the number of exmples/ demonstration
        full_prompt = ict_example*args.number_of_example
        # add the query
        full_prompt = full_prompt + 'This is a photo of Max: <image>. This is another photo of Max: '
