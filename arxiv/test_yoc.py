...
def get_args():
    ...
    parser.add_argument('--token_len', type=int, default=2)
...
if __name__ == '__main__':
    ...
    if args.reverse:
        # sks_prompt = f"{personalized_tokens[0]} is {''.join(personalized_tokens[1:args.token_len+1])}."
        sks_prompt = f"{''.join(personalized_tokens[1:][-args.token_len:])}."
    else:
        sks_prompt = f"{''.join(personalized_tokens[1:args.token_len+1])}."
    ...
    for i in tqdm(range(0, config_test.num_images, config_test.batch_size)):  # Step through by batch size
        ...
        if args.finetune:
            save_path = os.path.join(str(config_test.save_dir), config.exp_name, config_test.iteration+'ft', config.sks_name)
        else:
            save_path = os.path.join(str(config_test.save_dir), config.exp_name, config_test.iteration, config.sks_name)
        if args.reverse:
            index, image = save_generated_images(pixel_values, prompt_short, f'./generated_images/{config.sks_name}-without-sks-reverse/{args.token_len}', config.sks_name, index, img_size=args.img_size)
        else:
            index, image = save_generated_images(pixel_values, prompt_short, f'./generated_images/{config.sks_name}-without-sks/{args.token_len}', config.sks_name, index, img_size=args.img_size)
